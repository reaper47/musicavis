...
class Practice(models.Model):
    ...
    def format_total_practice_time(self) -> str:
        ...
        total_num_minutes = reduce(
            lambda x, y: x + y, [x.minutes for x in self.exercises.all()]
        )
    ...
    def __get_entities_from_db(self, model):
        ...
        for key, table, entities in loops:
            entities_in_db = [
                table.objects.filter(name=x.name).first() for x in entities
            ]
            all_entities[key] = [
                x if x else y for x, y in zip(entities, entities_in_db)
            ]

        for exercise in model.exercises:
            entity = Exercise.objects.filter(
                name=exercise.name,
                bpm_start=exercise.bpm_start,
                bpm_end=exercise.bpm_end,
...
class Exercise(models.Model):
    ...
    def __eq__(self, other):
        return (
            ...
            and self.bpm_start == other.bpm_start
            and self.bpm_end == other.bpm_end
