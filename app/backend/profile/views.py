...
def export_practices_view(request):
    ...
    request.user.profile.launch_task(
        "export_practices",
        "Exporting practices...",
...
def settings_access_view(request):
    ...
    if request.method == "POST":
        ...
        if change_email_form.is_valid() and is_password_correct:
            ...
            messages.info(
                request,
                "Your email has been updated. A confirmation link has been sent to you.",
            )
            return redirect(reverse("app:profile.settings"))
        ...
        errors = (
            list(change_password_form.errors.items())
...
def settings_practice_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            instruments = [
                Instrument.objects.get(name=name.title())
                for name in form.cleaned_instruments
                if name != "None"
            ]
            request.user.profile.update_instruments_practiced(instruments)

            instruments_string = (
                ", ".join([x.name for x in instruments]) if instruments else "None"
            )
            messages.info(
                request,
                f"Your instruments practiced have been set to {instruments_string}.",
            )
            return redirect(reverse("app:profile.settings"))
...
def settings_profile_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            request.user.profile.update_email_preferences(
                is_features, is_practicing, is_promotions
            )
