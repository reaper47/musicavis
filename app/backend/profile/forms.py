...
class SelectFileTypeForm(forms.Form):
    filetype = forms.ChoiceField(
        label="Select your preferred file format",
        widget=forms.Select,
...
class ChangeEmailForm(forms.Form):
    ...
    def clean_current_password(self):
        ...
        if User.objects.filter(
            email=self.cleaned_data.get("new_email", "null")
        ).first():
...
class SelectDefaultInstrumentForm(forms.Form):
    instruments = forms.MultipleChoiceField(
        label="Select the instruments you practice", required=False, choices=[]
    )
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        choices = sorted(
            [(x.name.lower(), x.name.title()) for x in Instrument.objects.all()]
        )
...
class EmailPreferencesForm(forms.Form):
    practicing = forms.BooleanField(
        label="Anything related to practicing", required=False
    )
    ...
    def __init__(self, profile=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["practicing"].widget.attrs.update(
            {"class": "checkbox big-input-checkbox"}
        )
        self.fields["promotions"].widget.attrs.update(
            {"class": "checkbox big-input-checkbox"}
        )
