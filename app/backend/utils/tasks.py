...
def export_practices(self, profile_id, name: str, description: str, os: str, file_type: str):
    task = Task.objects.create(id=self.request.id, name=name, description=description, profile_id=profile_id)

    profile = User.objects.get(profile__pk=profile_id).profile
    file_name = profile.export_practices(NewLine.from_string(os), FileType.from_string(file_type))

    task.complete = True
    profile.add_notification(f"export_practice_task_{file_type}",
