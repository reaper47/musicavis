...
class ExportPractices:
    ...
    def __init__(self, username: str, practices: List[Practice], newline: NewLine):
        ...
        self.odfstyles = self._prepare_odfstyles()

    def _prepare_odfstyles(self) -> Dict:
        h1style = odf.style.Style(name=f"Heading 1", family="paragraph")
        h1style.addElement(
            odf.style.ParagraphProperties(attributes={"textalign": "center"})
        )
        h1style.addElement(
        ...
        bulletlistproperty.addElement(
            odf.style.ListLevelProperties(minlabelwidth="1fcm")
        )
        ...
        tablecontents.addElement(
            odf.style.TextProperties(fontfamily="Arial", fontsize="14pt")
        )

        widewidth = odf.style.Style(name="col1", family="table-column")
        widewidth.addElement(
            odf.style.TableColumnProperties(columnwidth="2.8cm", breakbefore="auto")
        )

        widewidth2 = odf.style.Style(name="col2", family="table-column")
        widewidth2.addElement(
            odf.style.TableColumnProperties(columnwidth="6.8cm", breakbefore="auto")
        )

        borderbottom = odf.style.Style(name="cell1", family="table-cell")
        borderbottom.addElement(
            odf.style.TableCellProperties(borderbottom="0.05pt solid #000000")
        )

        borderright = odf.style.Style(name="cell2", family="table-cell")
        borderright.addElement(
            odf.style.TableCellProperties(borderright="0.05pt solid #000000")
        )
    ...
    def _export_csv(self, fname: str):
        with open(fname, "w", newline=self.newline) as f:
            ...
            for practice in self.practices:
                ...
                exercises = "exercises: " + f",{self.newline}\t\t\t".join(
                    [
                        (f"{x.name},{x.bpm_start}bpm,{x.bpm_end}" f"bpm,{x.minutes}m")
                        for x in practice.exercises.all()
                    ]
                )
                improvements = "improvements: " + ",".join(
                    [x.name for x in practice.improvements.all()]
                )
                positives = "positives: " + ",".join(
                    [x.name for x in practice.positives.all()]
                )
    ...
    def _export_docx(self, fname: str):
        bullet = "List Bullet"
        ...
        with open(fname, "w", newline=self.newline):
            ...
            for practice in self.practices:
                ...
                [
                    document.add_paragraph(x.name, style=bullet)
                    for x in practice.goals.all()
                ]
                ...
                [
                    document.add_paragraph(x.name, style=bullet)
                    for x in practice.improvements.all()
                ]

                document.add_heading("Positives", 2)
                [
                    document.add_paragraph(x.name, style=bullet)
                    for x in practice.positives.all()
                ]

                document.add_heading("Notes", 2)
                document.add_paragraph(
                    "No notes." if not practice.notes else practice.notes
                )

            document.save(fname)

    def _export_excel(self, fname: str):
        ...
        for index, practice in enumerate(self.practices):
            dfs = [
                pd.DataFrame({"Goals": [x.name for x in practice.goals.all()]}),
                pd.DataFrame(
                    {
                        "Exercise Name": [x.name for x in practice.exercises.all()],
                        "Bpm Range": [
                            f"{x.bpm_start}-{x.bpm_end}"
                            for x in practice.exercises.all()
                        ],
            ...
            header = (
                f"Practice #{index + 1} - {practice.instrument.name.title()} - "
                f"{practice.date:%a, %B %m %Y}"
            )
            writer.sheets[sheet_name].merge_range(
                f"A{offset+1}:H{offset + 1}", header, merge_format
            )

            all_elements = [
                practice.goals.all(),
                practice.exercises.all(),
    ...
    def _export_xml(self, fname: str):
        ...
        for index, practice in enumerate(self.practices):
            ...
            self._create_name_nodes(practice_root, "Goals", practice.goals.all())
            ...
            self._create_name_nodes(
                practice_root, "Improvements", practice.improvements.all()
            )
            self._create_name_nodes(
                practice_root, "Positives", practice.positives.all()
            )
    ...
    def _create_name_nodes(self, root: xml.Element, name: str, elements):
        sub = xml.SubElement(root, name)
        for el in elements:
            subelement = xml.SubElement(sub, "Name")
            subelement.text = el.name

    def _export_txt(self, fname: str):
        with open(fname, "w") as f:
            for practice in self.practices:
                header = (
                    f"{practice.instrument.name.title()} "
                    f"({practice.date}){self.newline}"
                )
                goals = (
                    f"goals -> {', '.join([x.name for x in practice.goals.all()])}"
                    f"{self.newline}"
                )
                exercises = (
                    f"exercises ->{self.newline}\t"
                    + f"{self.newline}\t".join(
                        [
                            f"{x.name} {x.bpm_start}-{x.bpm_end}bpm {x.minutes}m"
                            for x in practice.exercises.all()
                        ]
                    )
                    + f"{self.newline}"
                ...
                improvements = (
                    f"improvements -> "
                    f"{', '.join([x.name for x in practice.improvements.all()])}"
                    f"{self.newline}"
                )
                positives = (
                    f"positives -> "
                    f"{', '.join([x.name for x in practice.positives.all()])}"
                    f"{self.newline}"
                )
    ...
    def _export_pdf(self, fname: str):
        ...
        for practice in self.practices:
            with doc.create(
                ...
                self._add_list_itemize_latex(doc, "Goals", practice.goals.all())
                ...
                self._add_list_itemize_latex(
                    doc, "Improvements", practice.improvements.all()
                )
                self._add_list_itemize_latex(doc, "Positives", practice.positives.all())
    ...
    def _export_json(self, fname: str):
        root = {}
        for index, practice in enumerate(self.practices):
            root[f"practice-{index + 1}"] = {
                ...
                "exercises": [
                    {
                        "name": x.name,
                        "bpm_range": f"{x.bpm_start}-{x.bpm_end}",
    ...
    def _export_odt(self, fname: str):
        ...
        for index, practice in enumerate(self.practices):
            header = odf.text.H(
                outlinelevel=1,
                stylename=self.odfstyles["h1"],
                text=(
                    f"[{practice.date:%a, %d %B %Y}] Practice #{index + 1} - "
                    f"{practice.instrument.name.title()}"
                ),
            )
            ...
            self._add_bullet_list_odf(
                doc, "Goals:", [x.name for x in practice.goals.all()]
            )

            exercises_para = self._bold("Exercises:")
            ...
            for exercise in practice.exercises.all():
                ...
                listitem_p = odf.text.P(
                    text=(
                        f"{exercise.name} - {exercise.bpm_start} to "
            ...
            self._add_bullet_list_odf(
                doc, "Improvements:", [x.name for x in practice.improvements.all()]
            )
            self._add_bullet_list_odf(
                doc, "Positives:", [x.name for x in practice.positives.all()]
            )

            notes = self._bold("Notes: ")
            teletype.addTextToElement(
                notes, practice.notes if practice.notes is not None else ""
            )
    ...
    def _add_bullet_list_odf(self, doc, title, elements):
        para = self._bold(text=title)
        bulletlist = odf.text.List(stylename=self.odfstyles["bulletlist"])
        for el in elements:
            listitem = odf.text.ListItem()
    ...
    def _export_ods(self, fname: str):
        ...
        table.addElement(
            odf.table.TableColumn(
                stylename=self.odfstyles["widewidth"], defaultcellstylename="ce1"
            )
        )
        table.addElement(
            odf.table.TableColumn(
                stylename=self.odfstyles["widewidth2"], defaultcellstylename="ce2"
            )
        )
        table.addElement(
            odf.table.TableColumn(
                stylename=self.odfstyles["widewidth2"], defaultcellstylename="ce3"
            )
        )
        table.addElement(
            odf.table.TableColumn(
                stylename=self.odfstyles["widewidth2"], defaultcellstylename="ce4"
            )
        )

        for index, practice in enumerate(self.practices):
            tr_header = self._add_row(table)
            self._add_string_cell(tr_header, "", border=self.odfstyles["borderbottom"])
            self._add_string_cell(
                tr_header, f"Practice #{index +1}", True, self.odfstyles["borderbottom"]
            )
            self._add_string_cell(
                tr_header,
            ...
            self._add_string_cell(
                tr_goals, "Goals:", True, self.odfstyles["borderright"]
            )
            [
            ...
            self._add_string_cell(
                tr_exercise, "Exercises:", True, self.odfstyles["borderright"]
            )
            exercises = practice.exercises.all()
            for index, exercise in enumerate(exercises):
                self._add_string_cell(tr_exercise, exercise.name)
                self._add_string_cell(
                    tr_exercise, f"{exercise.bpm_start}-{exercise.bpm_end} bpm"
                )
                ...
                if index + 1 != len(exercises):
                    tr_exercise = self._add_row(table)
                    self._add_string_cell(
                        tr_exercise, odf.text.P(), border=self.odfstyles["borderright"]
                    )

            tr_improvements = self._add_row(table)
            self._add_string_cell(
                tr_improvements, "Improvements:", True, self.odfstyles["borderright"]
            )
            [
            ...
            self._add_string_cell(
                tr_positives, "Positives:", True, self.odfstyles["borderright"]
            )
            [
            ...
            self._add_string_cell(
                tr_notes, f"Notes:", True, self.odfstyles["borderright"]
            )
    ...
    def _add_string_cell(self, row, text, bold=False, border=None):
        cell = odf.table.TableCell(valuetype="string", stylename=border)

        if bold:
            p = self._bold(text=text)
            cell.addElement(p)
        else:
