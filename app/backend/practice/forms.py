...
class NewPracticeForm(forms.Form):
    ...
    copy_previous_practice_checkbox = forms.BooleanField(
        label="Copy previous session", required=False
    )
...
class PracticeForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["notes"].widget.attrs.update(
            {"class": "textarea", "placeholder": "Text lines...", "rows": 5}
        )

        self._add_fields("goal", instance.goals.all(), "Goal...")
        self._add_exercise_fields(instance.exercises.all())
        self._add_fields("positive", instance.positives.all(), "Positive...")
        self._add_fields(
            "improvement", instance.improvements.all(), "What to improve..."
        )

    def _add_fields(self, label, components, placeholder=""):
    ...
    def _add_exercise_fields(self, exercises):
        ...
        for idx, exercise in enumerate(exercises):
            ...
            for label, value in zip(labels, values):
                ...
                if label in ["bpm_start", "bpm_end", "minutes"]:
                    ...
                    self.fields[field_name].widget.attrs.update(
                        {"class": "input input-change"}
                    )
                else:
                    self.fields[field_name].widget.attrs.update(
                        {"class": "input", "placeholder": "Exercise name..."}
                    )
        ...
        for label in labels:
            ...
            if label == "minutes":
                self.fields[field_name].widget.attrs.update(
                    {"step": "0.5", "value": "5.0", "min": "1.0"}
                )
            elif label == "bpm_start":
                self.fields[field_name].widget.attrs.update(
                    {"step": "1.0", "value": "60", "min": "30", "max": "400"}
                )
            elif label == "bpm_end":
                self.fields[field_name].widget.attrs.update(
                    {"step": "1.0", "value": "80", "min": "30", "max": "400"}
                )

    def clean(self):
        self._field_cleaning("goal")
        self._field_cleaning("positive")
        self._field_cleaning("improvement")
        self._field_cleaning_exercise()

    def _field_cleaning(self, label):
        self.cleaned_data[f"{label}s"] = set()
        i = self._field_cleaning_with_dict(0, self.cleaned_data, label)
        if self.data:
            self._field_cleaning_with_dict(i, self.data, label)

    def _field_cleaning_with_dict(self, i, data, label):
        components = set()
        field_name = f"{label}_{i}"
        ...
        self.cleaned_data[f"{label}s"] = self.cleaned_data[f"{label}s"].union(
            components
        )
        return i

    def _field_cleaning_exercise(self):
        self.cleaned_data["exercises"] = set()
        i = self._field_cleaning_exercises_with_dict(0, self.cleaned_data)
        if self.data:
            self._field_cleaning_exercises_with_dict(i, self.data)

    def _field_cleaning_exercises_with_dict(self, i, data):
        components = set()
        while data.get(f"exercise_{i}_name"):
            component = ExerciseData(
                name=data[f"exercise_{i}_name"],
        ...
        self.cleaned_data["exercises"] = self.cleaned_data["exercises"].union(
            components
        )
        return i
    ...
    def save(self, instance):
        ...
        self._update_field("goals", instance)
        self._update_field("improvements", instance)
        self._update_field("positives", instance)
        self._update_field("exercises", instance)

        instance.save()

    def _update_field(self, field_name, instance):
        if field_name == "goals":
        ...
        if field_name == "exercises":
            for exercise_tuple in self.cleaned_data[field_name]:
                ...
                obj = Exercise.objects.filter(
                    name=name, bpm_start=bpm_start, bpm_end=bpm_end, minutes=minutes
                ).first()
                if not obj:
