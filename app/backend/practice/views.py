...
def new_view(request):
    ...
    args = dict(
        ...
        is_practices_available=bool(request.user.profile.practices.all()),
        form=form,
    )
...
def session_view(request, practice_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save(practice)
            return JsonResponse(
                {"status_code": 200, "toast": "Changes have been saved."}
            )
    ...
    args = dict(
        title=f"Practice #{practice.id} ({practice.instrument.name})",
        ...
        timer_components=[
            ("Hours:", "num-hours", 24, 0, 0),
            ("Minutes:", "num-minutes", 60, 0, 5),
