...
def change_email(client, new_email, repeat_email, password):
    data = dict(
        new_email=new_email, repeat_email=repeat_email, current_password=password
    )
    return client.post(ACCESS_SETTINGS, data=data)


def change_username(client, new_username, password):
    data = dict(new_username=new_username, current_password=password)
    return client.post(ACCESS_SETTINGS, data=data)
...
def delete_post(client, is_wrong_password=False):
    ...
    return client.post(
        reverse("app:profile.delete_account"), data=dict(password=password)
    )
