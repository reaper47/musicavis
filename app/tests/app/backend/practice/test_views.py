...
data = {
    "goal_0": A_GOAL_NAME,
    "positives_0": A_POSITIVE_NAME,
    "improvement_0": AN_IMPROVEMENT_NAME,
    "notes": "notes",
    "exercise_0_name": "C-Arpeggio",
    "exercise_0_bpm_start": 60,
    "exercise_0_bpm_end": 75,
    "exercise_0_minutes": 5,
}
...
class PracticeViewsTests(TestCase):
    ...
    def test_new_practice_session_hide_copy_previous_practice(self):
        """
        GIVEN a user with no practice sessions
        WHEN the user creates a new practice session
        THEN the "copy previous practice" checkbox is not displayed
        """
        response = self.client.get(self.url_practice, follow=True)

        self.assertNotIn(b'id="copy-previous-session-container"', response.content)

    def test_new_practice_session_show_copy_previous_practice(self):
        """
        GIVEN a user with a practice
        WHEN a user creates a new practice
        THEN the "copy previous practice" checkbox is displayed
        """
        self.a_user.profile.new_practice("trombone")

        response = self.client.get(self.url_practice, follow=True)

        self.assertIn(b'id="copy-previous-session-container"', response.content)
    ...
    def test_new_practice_session_post(self):
        ...
        response = self.client.post(
            self.url_practice, data=dict(instrument="trombone"), follow=True
        )
    ...
    def test_update_model(self):
        ...
        response = self.client.post(
            reverse("app:practice.session", args=[practice.pk]), data=data, follow=True
        )
    ...
    def test_update_model_test_exercsises(self):
        ...
        new_data = {
            ...
            "exercise_0_bpm_end": 75,
            "exercise_0_minutes": 5,
            "exercise_1_name": "C-Arpeggio",
            "exercise_1_bpm_start": 60,
            "exercise_1_bpm_end": 75,
            "exercise_1_minutes": 10,
            "exercise_2_name": "C-Arpeggio",
            "exercise_2_bpm_start": 60,
            "exercise_2_bpm_end": 75,
            "exercise_2_minutes": 5,
            "exercise_3_name": "C-Arpeggio",
            "exercise_3_bpm_start": 70,
            "exercise_3_bpm_end": 75,
            "exercise_3_minutes": 5,
        }
    ...
    def test_delete_session_anonymous_user(self):
        ...
        response = self.client.delete(
            reverse("app:practice.session", args=[practice.pk])
        )
    ...
    def test_delete_session_only_for_user(self):
        ...
        self.assertEqual(
            User.objects.filter(username="bob").first().profile.practices.count(), 1
        )
        self.assertEqual(
            User.objects.filter(username=A_USERNAME).first().profile.practices.count(),
            0,
        )
    ...
    def test_list_practices_display_total_time_practiced(self):
        ...
        for i in range(5):
            name = chr(ord("C") + i)
            a_practice.exercises.add(
                Exercise.objects.create(name=name, bpm_start=i, bpm_end=i, minutes=25)
            )
    ...
    def test_view_practice_session_multiple_entries(self):
        ...
        response = self.client.get(
            reverse("app:practice.session", args=[practice.pk]), follow=True
        )
