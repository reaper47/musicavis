...
class PracticeDtoTests(TestCase):
    ...
    def test_json_to_model_full(self):
        ...
        complete_json = {
            'goals': [{'name': A_GOAL}],
            'exercises': [{'name': AN_EXERCISE, 'bpm_start': A_BPM, 'bpm_end': A_BPM, 'minutes': SOME_MINUTES},
            ...
            'notes': A_NOTE}
        model_expected = PracticeDTO(goals=[Goal(name=A_GOAL.lower())],
                                     improvements=[Improvement(name=AN_IMPROVEMENT.lower())],
                                     exercises=[Exercise(name=OTHER_EXERCISE.lower(), bpm_start=A_BPM,
                                                         bpm_end=A_BPM, minutes=SOME_MINUTES),
                                                Exercise(name=AN_EXERCISE.lower(), bpm_start=A_BPM,
                                                         bpm_end=A_BPM, minutes=SOME_MINUTES)],
                                     positives=[Positive(name=A_POSITIVE.lower())], notes=A_NOTE)
    ...
    def test_json_to_model_partial(self):
        ...
        partial_json = {
            'goals': [{'name': ''}],
            'exercises': [{'name': '', 'bpm_start': A_BPM, 'bpm_end': A_BPM, 'minutes': SOME_MINUTES},
                          {'name': AN_EXERCISE, 'bpm_start': '', 'bpm_end': A_BPM, 'minutes': SOME_MINUTES},
                          {'name': AN_EXERCISE, 'bpm_start': A_BPM, 'bpm_end': '', 'minutes': SOME_MINUTES},
                          {'name': OTHER_EXERCISE, 'bpm_start': A_BPM, 'bpm_end': A_BPM, 'minutes': ''}],
            'positives': [{'name': ''}],
            'improvements': [{'name': ''}],
            'notes': ''}
        model_expected = PracticeDTO(goals=[], exercises=[], positives=[], improvements=[], notes=None)
    ...
    def test_model_to_jsonable(self):
        ...
        obj_expected = {'goals': [A_GOAL], 'improvements': [AN_IMPROVEMENT],
                        'positives': [A_POSITIVE], 'notes': 'notes',
                        'exercises': [
