...
an_exercise = Exercise(
    name="C-Arpeggio", bpm_start=60, bpm_end=80, minutes=Decimal("6.00")
)
other_exercise = Exercise(
    name="D-Arpeggio", bpm_start=90, bpm_end=80, minutes=Decimal("4.00")
)
...
class ExportsTestCase(TestCase):
    ...
    def test_export_csv(self):
        data_expected = (
            f"{a_date},{an_instrument.name.title()}\n"
    ...
    def test_export_docx(self):
        ...
        text = "\n".join(
            [p.text for p in docx.Document(f"{EXPORTS_DIR}/{fname}").paragraphs]
        )
    ...
    def test_export_xlsx(self):
        data_expected = (
            f"Practice#1-PaperMills-{a_date:%a,%B%m%Y}Unnamed:1Unnamed:2Unnamed:3Unnamed:4Unnamed:5"
        ...
        data_actual = (
            str(df).replace(" ", "").replace("\n", "").replace("[3rowsx8columns]", "")
        )

        self.assertGreaterEqual(
            SequenceMatcher(None, data_expected, data_actual).ratio(), 0.85
        )
    ...
    def test_export_txt(self):
        data_expected = (
            f"{an_instrument.name.title()} ({a_date})\n"
            f"goals -> {a_goal.name}, {other_goal.name}\n"
    ...
    def test_export_odt(self):
        text_expected = (
            f"[{a_date:%a, %d %B %Y}] Practice #1 - {an_instrument.name.title()}Goals:{a_goal.name}"
    ...
    def test_export_ods(self):
        text_expected = (
            f"Practice #1{an_instrument.name.title()}{a_date:%a, %d %B %Y}Goals:{a_goal.name}"
    ...
    def test_open_then_delete_deletes_file(self):
        ...
        chunkSize = 16384
        wrapper = FileDeleteWrapper(
            filepath=filepath, filelike=open(filepath, "rb"), blksize=chunkSize
        )
