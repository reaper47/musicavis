...
from app.models.practice import Instrument, Goal, Practice, Improvement, Positive, Exercise
from app.tests.conftest import create_user_with_a_practice
...
def export_practices(user: User) -> ExportPractices:
...
class ExportsTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.export_practices = export_practices(cls.a_user)
    ...
    def test_export_xlsx(self):
        data_expected = (f'Practice#1-PaperMills-{a_date:%a,%B%m%Y}Unnamed:1Unnamed:2...Unnamed:5Unnamed:6Unnamed:'
                         '70NaNGoalsExerciseName...ImprovementsPositivesNotes10.0eatC-Arpeggio...holdpicklightergood'
                         'pickingthebrimwasheldlikeachamp21.0sleepD-Arpeggio...practiceslowerex8nexttimewasableto'
                         'focusthistimeNaN')

        fname = self.export_practices.export(FileType.XLSX)
        df = pd.read_excel(f'{EXPORTS_DIR}/{fname}', sheet_name='Practices')

        self.assertEqual(data_expected, str(df).replace(' ', '').replace('\n', '').replace('[3rowsx8columns]', ''))
